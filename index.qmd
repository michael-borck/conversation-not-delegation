...
Every chapter closes with **Take it to the AI**. A book about conversation should end each chapter by handing you one. Each section gives you a few questions worth arguing about, the keywords that let you steer a conversation you cannot yet phrase, and a starting prompt written out in full. Those prompts use RTCF (Role, Task, Context, Format), which @sec-rtcf covers properly; you do not need to have read that chapter first, because the four labels do most of the explaining themselves. Write all four if you are running a small model on your own machine, the kind with a few billion parameters that fits on a laptop; its defaults come from a narrower range, and the structure is doing work the model will not do for you. On a current frontier model the Task and Context lines are usually enough on their own, and the rest is a checklist for your own thinking. Either way, the first answer is where the conversation starts. Push back on it. That is the whole book in one habit.
...
- **Practise the frameworks.** The companion site includes three interactive tools that run in your browser: no login, no data stored. The **RTCF Prompt Builder** walks you through constructing prompts step by step using the framework from @sec-rtcf. The **RTCF Prompt Analyser** lets you paste any prompt and get instant feedback on which elements are present, partial, or missing. That is useful for diagnosing why a prompt gave a vague response. The **AI Readiness Assessment** is a short quiz that maps your current experience level and recommends which chapters to prioritise. See @sec-interactive-tools for details.
