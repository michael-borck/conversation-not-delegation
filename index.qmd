...
The academic literature has other names for what gets preserved when you do this. Philosophers and ethicists call it **agency**: your capacity to choose, weigh, and own the outcome. AI policy and system design call it **human-in-the-loop** (HIL): the principle that a person remains an active participant in decisions a system could otherwise automate. This book uses a different word, **conversation**, because the academic terms describe *what* you keep, and the conversational frame describes *how* you keep it. Same idea, three vocabularies. If you came in looking for guidance on human agency or HIL, you are in the right place. See @sec-quick-reference for the terms map.
...
This is not a guide to specific AI tools. It does not teach you how to use ChatGPT, Claude, or any particular platform. Interfaces change; the principles here do not. Where the book names a specific tool or describes how current models behave, those details will date faster than the surrounding ideas. The principles are the point.
...
A related warning: conversation, in this book, is not a string of zero-shot prompts run back-to-back. That is just delegation in volume. Real conversation requires pushback, course correction, and your judgement woven through each exchange. @sec-conversation-loop draws the distinction carefully.
...
Part 1 begins with the delegation trap, then gives you enough understanding of AI and large language models to recognise why that trap is so tempting. Part 2 establishes the principles that make AI conversations productive rather than passive: think first, work in a conversation loop, stay critical, and verify what matters. Part 3 turns those principles into practical prompting methods and techniques you can use immediately. Part 4 shows the approach working across disciplines, offers an optional advanced chapter on autonomous loops, and closes with the argument that matters most: the goal is not to get AI to do more, but to become more capable yourself.

By the end, you should be able to:

- decide whether a task calls for mechanical delegation, conversation, a more reliable non-AI tool, or no AI at all;
- frame a useful starting request without mistaking prompt structure for judgement;
- identify and redirect a plausible but unsuitable response;
- verify the claims that carry real consequences; and
- produce a record showing which parts came from the tool, which decisions were yours, and why you can defend the result.

## Evidence of progress, part by part

| Part | What you should be able to show |
|---|---|
| Understanding the landscape | Classify a task, distinguish a model from its surrounding tools, and explain why a plausible answer needs checking |
| Principles | Preserve a first attempt, identify a consequential error, and complete Verify–Explain–Test against independent evidence |
| Methodology | Write a scoped brief, choose a technique for a specific difficulty, and place a check between dependent steps |
| Putting it together | Produce an owned decision with source support, a rejected or revised suggestion, and a boundary test |

Keep a small learning record as you read. Start with one task and your unaided attempt; add the evidence or correction each part makes possible. The worked community workshop in @sec-across-disciplines shows the complete record and supplies a parallel task with an answer check.

You can complete the independent activities without an AI account. “Unaided” here means without handing the judgement to AI; calculators, assistive technology, notes, dictation, and help accessing the material are welcome. Shorten a session or explain with a diagram if that lets you inspect the reasoning more clearly. The advanced loop chapter is optional, not a gate to completing the book.
...
Most chapters close with a short practice section. Selected chapters provide complete RTCF starting prompts; other chapters ask you to begin without AI, evaluate evidence, or transfer the method to your own work. RTCF (Role, Task, Context, Format) is covered properly in @sec-rtcf and summarised in @sec-quick-reference. The structure is scaffolding, not a ritual. Use only the elements that clarify your task, then push back on the first answer.
...
- **Explore the source.** The full source is on [GitHub](https://github.com/michael-borck/conversation-not-delegation), including every chapter, the build system, and the revision history. [DeepWiki](https://deepwiki.com/michael-borck/conversation-not-delegation) provides an AI-navigable view of the repository.
- **Browse all books.** This book is part of a series. See all titles at [books.borck.education](https://books.borck.education).

The online version is always the most current. The printed and ebook editions are updated periodically. Use the free online edition and its downloads for the latest available revision. A purchased ebook is not guaranteed to update automatically.

## The Sibling Book

| Route | Start here | Continue with |
|---|---|---|
| Concepts before code | Conversation, Not Delegation | Converse Python, Partner AI; apply its method alongside the appropriate programming level |
| Python from the beginning | Think Python, Direct AI | Code Python, Consult AI, then Ship Python, Orchestrate AI |
| Web development | Think Python, Direct AI | Build Web, Guide AI |
...
You are reading *Conversation, Not Delegation*, the general methodology that underpins the series.
