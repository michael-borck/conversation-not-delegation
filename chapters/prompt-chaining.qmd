...
## Practise the Checkpoints

Use one ongoing task and preserve the intermediate answers rather than jumping to the end:

1. Take a chain you have run and ask the AI which link carried the most weight. That is where your review time should have gone, and it is rarely where it went.
2. In a long conversation, ask what it has dropped since the early exchanges. Context loss is invisible until you ask about it directly.
3. Ask it to design a chain for a task you describe, then ask which single link you should not automate. Its answer to the second question is more useful than its answer to the first.

**Keywords to steer with.** guided workflow · sequential chain · interactive chain · the pause between steps · iterative refinement · format shifting · perspective shifting · context window · drift · summarise and hand off · one task per prompt

**A starting prompt.**

> **Role:** You are a workflow designer who breaks big requests into steps and insists on a review point between each one.
>
> **Task:** Design a prompt chain for the task I describe, and mark exactly where I must stop and check before continuing.
>
> **Context:** I have read a chapter on chaining, on the pause between links, and on how context drifts across a long conversation. The task is [describe it, and say how long it usually takes me]. Assume I will be tempted to skip the review points.
>
> **Format:** Numbered links with the prompt for each. After every link, a "check this before continuing" line naming the specific thing that goes wrong if I skip it. End with the one link where a mistake would be hardest to notice later.
