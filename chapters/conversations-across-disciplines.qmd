...
This chapter walks through four complete examples across four different domains. Each one follows a realistic starting point through a real conversation to a result the human owns. The summaries below capture the shape of each conversation rather than reproducing every exchange in full. One scenario (the team lead's, below) includes a sample of the actual back-and-forth at its steering moment, to show what that turn looks like in practice.
...
::: {.callout-note title="What the steering moment actually looks like"}
The exchange after the textbook plan landed:

> **You:** This looks like every migration plan ever written. We ran almost exactly this last time and went 40% over. What specifically goes wrong in phase two when you don't have a dedicated data engineer?
>
> **AI:** Without a dedicated data engineer, phase two (mapping) typically fails in five specific ways:
>
> 1. **Schema drift goes unnoticed.** Source-system fields change mid-mapping and no one owns watching for it.
> 2. **Domain knowledge gets fragmented.** Team members map different tables on their own assumptions, producing inconsistent transformations.
> 3. **Edge cases become rework.** Without one person holding the full mapping model, unusual records get handled inconsistently and surface during validation.
> 4. **Validation criteria get written after the fact.** The team validates against what came out, not against what should have come out.
> 5. **The mapping document becomes stale.** It is treated as a deliverable, not a living artefact, so by phase three it no longer reflects what was built.
>
> **You:** Items 2 and 3 are exactly what happened last time. What would a "dedicated mapping sprint" look like for a team of four with no full-time engineer?

*The first prompt got a textbook plan. The second prompt named the constraint that actually mattered. Same model, same session. The difference was something only he could bring.*
:::
...
1. Run the loop on a real task from your own field and note the exact point where you rejected the AI's output. That rejection point *is* the method; everything else is scaffolding around it.
2. Ask it to walk your discipline's version of one of the four worked examples, then check whether its account of how your field actually works rings true. Where it does not is where you will need to supply the most context.
3. Ask what it would need to know about your situation to stop giving you the generic version. Then supply exactly that, and run the same request again.

**Keywords to steer with.** brainstorm, ideate, iterate, amplify across domains · domain knowledge as the input · the steering decision · rejecting the first draft · thinking versus building phases · the human as bridge

**A starting prompt.**

> **Role:** You are a practitioner in my field who has used AI on this kind of work and is willing to say where it did not help.
>
> **Task:** Walk me through how you would run the conversation loop on the task I describe, stopping at the point where your own judgement would override the model.
>
> **Context:** I have read a chapter showing the loop applied across writing, analysis, planning and communication, and arguing that the turning point is always a moment where the human rejects the output for a reason only they hold. My field is [name it]. My task is [describe it].
>
> **Format:** The four stages in order, with what you would ask at each. Mark the stage where you would expect to reject the AI's output, and say what knowledge that rejection would rest on.
