...
**Large language models** are the most recent breakthrough. Trained on billions of text examples to predict what word comes next, they turned out to learn far more than grammar along the way. They learned facts, reasoning patterns, writing styles, and the ability to follow complex instructions. ChatGPT, Claude, and Gemini are all large language models.

Each of these builds on the one before it. Rule-based systems gave way to machine learning, which gave way to deep learning, which produced large language models. The progression is toward systems that need less hand-coding and can handle more ambiguity.
