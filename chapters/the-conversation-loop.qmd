...
## Same Method, Different Calibration

The methodology in this book works across the full range of models you will encounter, but the texture of the conversation shifts depending on what you are using. The loop is constant. The amount of scaffolding you bring to it is not.

On a smaller or local model (anything you run yourself, anything below the current frontier), heavier scaffolding pays off. A clear role, an explicit task, well-chosen context, and a specified format are doing real work. Without them, the model guesses, and its guesses come from a smaller distribution of patterns. Structure compensates for capability.

On a current frontier model running inside a thick product harness (@sec-what-are-llms), some of that scaffolding becomes optional. The model's defaults are strong, and the harness has already done part of the role-setting before your prompt arrives. A sharply specified task with the right context often outperforms an elaborate persona invocation.

The temptation is to read this as "RTCF is for beginners; frontier users can skip it." That misses the point. RTCF (@sec-rtcf) is a thinking discipline as much as a prompting one. Asking *who would the right expert be, what exactly am I asking for, what do they need to know, how should the answer be shaped* sharpens your own thinking before the model touches it. That is valuable whether or not the model still needs all four to perform well.

The practical rule: scaffold heavily when the model is small or the stakes are high. Scaffold lightly when the model is capable and the harness is doing work for you. The model class changes how you open the conversation. It does not change whether you should have one.
...
When you ask AI to produce something (a draft, a plan, an analysis, a piece of code), there are two outputs in front of you. There is the artefact: the thing you asked for. And there is the natural language around it: the model's framing, its reasoning, its caveats, its assumptions about what you actually want.

Most people read the artefact and skim the language. That is exactly backwards.

The artefact is downstream. It is what the model produced given its understanding of your request. If the understanding is wrong, the artefact will be wrong in ways the artefact alone will not reveal. You will get something that looks right, that you cannot quite explain, and that subtly misses what you actually needed. The natural language is where the model tells you what understanding it arrived at. That is where you catch misalignment, before it hardens into a deliverable you then have to take apart.

The implication is concrete: read the words. If the model's explanation of what it did does not match what you wanted, the artefact is wrong even if it looks correct. Steer the conversation by responding to the words, not just by revising the artefact. If the framing is verbose, tell it to be brief. If it is making assumptions you do not share, name them. If it is wrong, do not just delete the output. Explain why it is wrong, and let the next attempt be informed by a shared understanding rather than a fresh start.

This holds whether you have deep expertise or none at all. The expert uses the conversation to direct: here is the constraint, here is what you missed, this is why the obvious approach is wrong. The newcomer uses it to understand: explain that, break this down, what would happen if. Both are reading the words. Both are using the natural language as the place where alignment happens. The artefact is what arrives at the end. It is not what you should be watching most closely along the way.
...
1. Take a prompt you wrote recently and ask the AI whether it describes your *problem* or your *attempted fix*. Most people discover the second.
2. Run the same question through a frontier model and a small local one. The loop is identical; the scaffolding is not. Notice which one made you think harder.
3. Before it answers anything, ask it to read back what it understood your request to be. Misalignment shows up in the framing long before it shows up in the artefact.

**Keywords to steer with.** brainstorm, ideate, iterate, amplify, repeat · problem versus attempted fix · pushback · the two-chat workflow · curation between sessions · unchecked chaining · the viva · reading the words rather than the artefact · scaffolding to the model
