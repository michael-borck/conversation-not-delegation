...
Choose a task you know well enough to judge, then test these distinctions:

1. Ask for a citation on something in your own field, then go and look it up. This is the cheapest possible way to feel what "fluency and accuracy come from the same process" actually means.
2. Ask it to explain the difference between interpolation and retrieval using an example from your work rather than a generic one.
3. Place three tasks you did this week on the average/precise by small/large grid, then ask the AI where it would put them. The disagreements are worth arguing out.

**Keywords to steer with.** next-token prediction · parameters · training cutoff · hallucination · interpolation versus retrieval · unidirectional attention · average versus precise · small versus large · harness · system prompt · safety filtering · custom instructions

**A starting prompt.**

> **Role:** You are a machine-learning researcher who explains models to non-technical people and refuses to use the word "magic."
>
> **Task:** Take the task I describe and tell me whether it needs interpolation or retrieval, and therefore whether a language model is the right tool at all.
>
> **Context:** I have read a chapter arguing that LLMs interpolate rather than retrieve, and that most mistakes come from treating them as lookup engines. The task is [describe it, including how precise the answer must be and what happens if it is wrong].
>
> **Format:** A verdict (interpolation, retrieval, or a mix), the reasoning in three sentences, and, where it is retrieval, what tool or source should be doing that part instead of the model.
