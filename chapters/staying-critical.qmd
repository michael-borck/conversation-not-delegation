...
### How the Conversation Loop Fights All Four
...
Apply the table above to a real output, then probe the warning signs:

1. Ask the AI to evaluate the same piece of your work twice: once with "what do you think?" and once with "what are the three weakest points?" The gap between the two answers is the sycophancy you normally never see.
2. Run the inherited-defaults exercise from this chapter, then ask the model what assumptions it made about the character that it would not have made for a different role. It can surface its own defaults, but only when asked.
3. Ask it to argue the AI Dismissal Fallacy from both sides. Uncritical rejection and uncritical acceptance are the same failure wearing different clothes.

**Keywords to steer with.** fluency versus accuracy · transparency · Gell-Mann amnesia · AI dismissal fallacy · genetic fallacy · sycophancy · inherited defaults · yellow and red flags · provenance · de-identify before you prompt · the self-check

**A starting prompt.**

> **Role:** You are a hostile reviewer who has been told that praising this work would be a professional failure.
>
> **Task:** Find the three weakest points in what I paste, and say what would have to be true for each one to sink the whole thing.
>
> **Context:** I have read a chapter on sycophancy, and on why "what do you think?" reliably produces flattery first and gentle criticism second. Here is my work: [paste it]. Do not open with anything positive. If you catch yourself hedging, say so, then stop hedging.
>
> **Format:** Three weaknesses, ordered by how much damage each does. For each: the flaw, what would have to be true for it to be fatal, and the cheapest thing I could do to find out.
